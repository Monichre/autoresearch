...
image = (
    ...
    .uv_pip_install([
        "anthropic>=0.40.0",     # kept as optional fallback
        "groq>=0.11.0",          # primary LLM provider for agent_loop
        "kernels>=0.11.7",
        "matplotlib>=3.10.8",
        "numpy>=2.2.6",
        "pandas>=2.3.3",
...
def _call_llm(best_train_py: str, history_rows: list) -> tuple[str, str]:
    ...
    Uses Groq (primary, free tier) with a fallback model on rate-limit.

    Returns (modified_train_py, description) where description is the
    [phase] tag line generated by the LLM.

    Uses a patch format (<<<OLD>>>/<<<NEW>>>) so the LLM only needs to
    output the changed lines — keeping the request well under Groq's
    12 000-token free-tier limit.
    """
    import time
    ...
    # Fallback chain: gemma2-9b-it has 15k TPM (higher headroom than llama 12k).
    # qwen-qwq-32b was decommissioned by Groq.
    # llama-3.1-8b-instant only has 6k TPM — can't fit our ~9k prompt.
    model_chain = [
        "gemma2-9b-it",     # 15k TPM — highest free-tier headroom
        GROQ_MODEL,         # llama-3.3-70b-versatile  (12k TPM)
    ]
    ...
    def _try_model(model: str) -> tuple[str, str]:
        """Call one model and return (modified_train_py, description). Raises on failure."""
        print(f"[_call_llm] model={model}")
        resp = client.chat.completions.create(
            ...
            max_tokens=1500,   # patch is small — description + ~30 changed lines
            temperature=0.7,
        )
    ...
    # Retry schedule: quick retries first (per-minute TPM), then long waits
    # (per-hour or per-day quotas). Designed for unattended overnight runs.
    #   attempts 0-2 : sleep 70s each  → clears per-minute window
    #   attempts 3-5 : sleep 1800s each → clears any per-hour window
    #   attempt  6+  : sleep 3600s each → waits for daily quota refresh
    ...
    MAX_RATE_RETRIES = 10   # up to ~10h of waiting before giving up on a model
    ...
    for model in model_chain:
        for attempt in range(MAX_RATE_RETRIES):
            try:
                ...
                print(f"[_call_llm] {model} rate-limited (attempt {attempt+1}), "
                      f"sleeping {wait}s...")
                time.sleep(wait)
            except BadRequestError as e:
                print(f"[_call_llm] {model} bad request — skipping model: {e}")
                break   # hard error on this model, try next
            except ValueError as e:
                print(f"[_call_llm] {model} patch parse error: {e}")
                break   # patch malformed — try next model
        else:
            print(f"[_call_llm] {model} exhausted {MAX_RATE_RETRIES} retries, trying next model")

    raise RuntimeError(
        f"All models in chain exhausted after retries: {model_chain}. "
        "Check Groq daily quota or upgrade plan."
    )
